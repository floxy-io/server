...
var Spark = function Spark(ctx, x, y, config) {
    ...
    this.c = [
        Math.floor(Math.random()*155)+config.cVar[0],
        Math.floor(Math.random()*80)+config.cVar[1],
        Math.floor(Math.random()*80)+config.cVar[2]
    ];
...
Spark.prototype.getAlpha = function() {
    return Math.sqrt(this.life/this.lifeOrig) / 2 +((Math.random()-0.5)/this.config.sparkBlink);
};
